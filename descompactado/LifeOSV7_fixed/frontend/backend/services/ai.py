...
def _extract_main_profession(prof_text: str) -> str:
    ...
    prefixes = [
        ...
        "agricultor", "veterinário", "piloto", "atleta", "coach", "personal",
    ]
...
def _call(client: Groq, system: str, user: str,
          max_tokens: int = 1200, fast: bool = False) -> str:
...
def _log_generation(uid: str, gen_type: str, prompt: str, content, ti: int = 0, to: int = 0):
    ...
    success = not content.get("parse_error", True) if isinstance(content, dict) else True
...
def generate_life_plan(uid: str, context: Dict = None) -> Dict:
    ...
    try:
        raw    = _call(_client_planning, system, user_prompt, max_tokens=2000)
        result = _parse_json(raw)
        _log_generation(uid, "life_plan", user_prompt, result, r.usage.prompt_tokens, r.usage.completion_tokens)
        return result
    except Exception as e:
