...
def build_context(uid: str) -> Dict[str, Any]:
    ...
    # 1. User profile
    profile = fetch_one(query(db.table("user_profiles").eq("user_id", uid).limit(1)))
    profession_type = profile.get("profession_type", "gen") or "gen"
    profession = profile.get("profession", "") or ""  # Full profession text (e.g., "professor de matemática")
    energy_pattern = profile.get("energy_pattern", "normal") or "normal"
    
    # 1.1. NEW: Get profession attributes (universal model)
    ...
    answers_summary = {
        qid: ans.get("parsed_value", ans.get("raw_answer", ""))[:150] 
        for ans in answers 
        for qid in [ans.get("question_id")] if qid
    }
    ...
    streak = sum(1 for c in checkins[:7] if c.get("done", False))
